...
This example shows how a TPU cluster with v5p machines and topology 2x2x1 can be created. The example also includes a `tpu-multislice.yaml` that creates a kubernetes service and job. The job includes commands to install `jax` and run a simple command using jax, on the TPU.
...
* `tpu_topology`: The TPU topology desired. Topology is the number and physical arrangement of the TPU chips in a TPU slice. For v5p, topology is specified in 3D format (AxBxC), for example `2x2x1`.

> **NOTE:** TPU v5p does not require multi-NIC networking. This blueprint uses a single VPC, unlike the v6e and 7x blueprints which require dual VPCs.
...
* TPU v5p is available in the following zones: `europe-west4-b`, `us-central1-a`, `us-east5-a`.

* Minimum GKE version for TPU v5p: `1.28.3-gke.1024000`.
...
1. In the [`examples/gke-tpu-v5p/gke-tpu-v5p-deployment.yaml`](https://github.com/GoogleCloudPlatform/cluster-toolkit/blob/main/examples/gke-tpu-v5p/gke-tpu-v5p-deployment.yaml) file, replace the following variables in the `terraform_backend_defaults` and `vars` sections to match the specific values for your deployment:

   * `bucket`: the name of the Cloud Storage bucket you created in the previous step.
   * `project_id`: your Google Cloud project ID.
   * `region`: the compute region for the cluster (e.g., `us-east5`).
   * `zone`: the compute zone for the TPUs (e.g., `us-east5-a`).
   * `num_slices`: the number of TPU slices to create.
   * `machine_type`: the machine type of the TPU (default: `ct5p-hightpu-4t`).
   * `tpu_topology`: the TPU placement topology for pod slice node pool (3D format, e.g., `2x2x1`).
   * `static_node_count`: the number of TPU nodes in your cluster. For v5p, this is calculated as: total chips from topology / 4 chips per `ct5p-hightpu-4t` node. For example, `2x2x1` = 4 chips / 4 = 1 node.
   ...
   * `reservation`: the name of the compute engine reservation of TPU v5p nodes.
...
1. To use on-demand capacity, you can remove the reservation usage by making the following changes.
   1. Remove the `reservation` variable from the [`gke-tpu-v5p-deployment.yaml`](https://github.com/GoogleCloudPlatform/cluster-toolkit/blob/main/examples/gke-tpu-v5p/gke-tpu-v5p-deployment.yaml) file.
...
1. Deploy the blueprint to provision the GKE infrastructure using TPU v5p machine types:
...
This repository also includes an advanced blueprint, `gke-tpu-v5p-advanced.yaml`, designed for production-ready workloads. It builds on the basic blueprint by adding several key features:
...
1. Ensure you have completed steps 1-7 from the "Create a cluster" section above. The same `gke-tpu-v5p-deployment.yaml` file can be used.

1. In the final deploy command, simply point to the `gke-tpu-v5p-advanced.yaml` blueprint instead.

    ```sh
    cd ~/cluster-toolkit
    ./gcluster deploy -d \
    examples/gke-tpu-v5p/gke-tpu-v5p-deployment.yaml \
    examples/gke-tpu-v5p/gke-tpu-v5p-advanced.yaml
    ```
...
This blueprint supports [Kueue](https://kueue.sigs.k8s.io/), a kubernetes-native system for managing quotas and job queuing. This is enabled by default in the advanced blueprint (`gke-tpu-v5p-advanced.yaml`).
...
2. **Submit a Job:** To submit a job to the queue, add the label `kueue.x-k8s.io/queue-name: user-queue` to your Job or JobSet manifest.
    ...
    ```sh
    kubectl create -f ~/cluster-toolkit/examples/gke-tpu-v5p/kueue-job-sample.yaml
    ```
...
1. Create the resources:
    ...
    ```sh
    kubectl create -f ~/cluster-toolkit/examples/gke-tpu-v5p/tpu-multislice.yaml
    ```
...
1. Display logs of either of the pods using:
    ...
    This should display `Global device count: 4` at the end of the logs which is the number of TPU chips across all of the nodes in a single TPU slice (4 chips for a `2x2x1` topology with `ct5p-hightpu-4t`).
...
2. [TPU v5p](https://cloud.google.com/tpu/docs/v5p)
...
2. In the `gke-tpu-v5p-advanced.yaml`:
...
1. Ensure the GKE cluster is configured to support standard Persistent Disks (the Hyperdisk CSI driver runs automatically once enabled). Verify the `gke-tpu-v5p-cluster` module setting `enable_persistent_disk_csi: true` is present.
...
1. In the `gke-tpu-v5p-cluster` module settings, ensure `enable_filestore_csi: true` is set.
